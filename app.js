...
function updatePDPView(p){
  ...
  if(mainImg){
    ...
    visual.insertBefore(newImg, visual.firstChild);
  }
  // Update tag
  const tag = document.getElementById('pdpColorTag');
  if(tag){
    const counter = imgs.length > 1
      ? `<span class="pdp-image-counter">${currentPDPImageIdx+1} / ${imgs.length}</span>`
      : '';
    tag.innerHTML = `<span class="pdp-color-name">${esc(c.name)}</span>${counter}`;
  }
  // Show/hide nav arrows based on image count
  const prev = document.getElementById('pdpNavPrev');
  const next = document.getElementById('pdpNavNext');
  if(prev) prev.style.display = imgs.length > 1 ? '' : 'none';
  if(next) next.style.display = imgs.length > 1 ? '' : 'none';
  // Update color swatches
  document.querySelectorAll('#pdpColors .pdp-col').forEach((el, i)=>{
    el.classList.toggle('sel', i === currentPDPColorIdx);
  });
  // Update thumb active state — uniquement les miniatures de la couleur courante
  document.querySelectorAll('#pdpThumbs .pdp-thumb').forEach((el, i)=>{
    el.classList.toggle('active', i === currentPDPImageIdx);
  });
}

function goToContact(){
  closePDP();
  // If on produits page, navigate to contact page
...
/* ─── REVEAL ANIMATION ─── */
// Activer les animations seulement si JS fonctionne
document.documentElement.classList.add('js-ready');
function checkReveals(){
  document.querySelectorAll('.reveal:not(.vis)').forEach(el=>{
    const r = el.getBoundingClientRect();
    if(r.top < window.innerHeight + 60) el.classList.add('vis');
  });
}
...
document.addEventListener('keydown', e=>{
  if(e.key==='Escape'){ closePDP(); closeMenu(); }
});
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // Open PDP from URL ?id=X if on produits page
  const params = new URLSearchParams(location.search);
  const idParam = params.get('id');
  if(idParam !== null && hasPDP){
